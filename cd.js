  ...
  window.starrezCalendar = (month, year, options) => {
    function createCalendar(month, year) {
      ...
      if (elem) {
        ...
        prev.classList.add("ui_prev_button");
        ...
        prev.classList.add("ui_next_button");
        ...
        today.onclick = () => {
          ...
          createCalendar(d.getMonth() + 1, d.getFullYear()).then((calendar) =>
            calendar.querySelector(`[data-date="${d.toDateString()}"]`).focus()
          );
        };
      ...
      function buildTable(month, year) {
        ...
        while (d.getMonth() === mon) {
          ...
          button.onclick = function () {
            ...
            // and focus on it.
            createCalendar(month, year).then((calendar) =>
              calendar.getElementsByClassName("current")[0].focus()
            );
          };
          ...
          button.onkeydown = function (e) {
            ...
            if (tbody.rows[rowIndex] && tbody.rows[rowIndex].cells[cellIndex]) {
              ...
              if (
                targetCell &&
                targetCell.getElementsByTagName("button").length
              ) {
      ...
      function disablePreviousButton() {
        const currentToday = new Date(options.today);

        if (
          month <= currentToday.getMonth() + 1 &&
          year === currentToday.getFullYear()
        ) {
          options.elem.getElementsByClassName("ui_prev_button")[0].disabled =
            "disabled";
        } else {
          options.elem.getElementsByClassName("ui_prev_button")[0].disabled =
            null;
